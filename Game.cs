...
public class Game
{
    ...
    private void PerformTurn(GameMode mode)
    {
        if (mode == GameMode.EvE || (mode == GameMode.PvE && isPlayer1Turn))
        {
            AIShot();
        }
        else
    ...
    private void Render(GameMode mode)
    {
        Console.Clear();

        ShowBattlefield(player1.Name + "'s Battlefield", player1.Field, mode == GameMode.PvP || mode == GameMode.PvE || mode == GameMode.EvE);
        ShowBattlefield(player2.Name + "'s Battlefield", player2.Field, mode == GameMode.PvP || mode == GameMode.EvE);
    }
